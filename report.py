...
class ReportGenerator:
    def __init__(self, model_objects, networkx_graphs, image_with_objects=None):
        if image_with_objects is not None:
            ...
            Image.fromarray(image_with_objects).save(self.image_filename)
    ...
    @staticmethod
    def _print_found_object(found_object, index):
        return """
        <tr>
        <td>{}</td>
        <td>{}</td>
    ...
    def _print_found_objects(self):
        ...
        """.format("\n".join(
            self._print_found_object(found_object, index + 1) for index, found_object in enumerate(self.found_objects)))

    def _get_found_object_types(self):
        return set(found_object["name"] for found_object in self.found_objects)
    ...
    def _count_found_objects(self, object_type=None):
        if object_type:
            names = [found_object["name"] for found_object in self.found_objects]
    ...
    def generate_pdf_report(self, image):
        ...
        pdf.write_html(self._print_found_objects())
        return pdf
        # TODO Draw object numbers
